/**
 * Minted bearer — short-lived, per-tenant authentication token. Stateless
 * HMAC-SHA256 signature; no KV round-trip to verify.
 *
 * Token shape:  mbr.v1.<tenantIdB64url>.<expSec>.<sigB64url>
 *
 * The gateway typically asks the middleware to mint these via service binding
 * (middleware is the canonical signer); this module is kept here so the
 * gateway can also verify locally and, when the future middleware-less
 * happy-path needs it, mint directly. Centralize all mbr.v1 logic here.
 */
